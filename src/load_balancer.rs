...
impl LoadBalancer {
    pub async fn run(addr: &str, workers: Vec<WorkerConfig>) -> Result<(), LoadBalancerError> {
        ...
        let state = LoadBalancerState {
            address,
            workers,
            strategy: Arc::new(RwLock::new(BalancingStrategy::default())),
        };
    ...
    //     let tower_service =
    //         tower::service_fn(move |req: Request<_>| async move { handle(req).await });
    //     let hyper_service = hyper::service::service_fn(move |mut request: Request<_>| {
    //         request.uri_mut();
    ...
    //             if let Err(err) = http1::Builder::new().preserve_header_case(true).title_case_headers(true).sen
    //         })
    //     }
    // }
}
...
pub async fn handle_request(
    ...
    // };
    let chosen_worker = workers.first().unwrap();
